...
if(!empty($obj['status'])) {
		...
		while($result = mysqli_fetch_assoc($id_category)) {
			$cek = $db -> selectpage('tb_report', 'id_category="'.$result['id_category'].'" AND (status ="'.$status1.'")', 'time_post', 'DESC', $start, $limit);
	...
	// 			$cek = $db->selectpage('tb_report', 'status="process" || status ="process"', 'id_report', 'DESC', $start, $limit);
