...
if (isset($_POST['name_manager']) && isset($_POST['username']) && isset($_POST['password']) && isset($_POST['id_estate_cordinator']) && isset($_POST['id_contractor']) && isset($_POST['manager_job']) && isset($_POST['email']) && isset($_POST['no_telp'])) {

    $unique_id = UUID::guidv4();
    $name_manager = mysqli_real_escape_string($db->query, $_POST['name_manager']);
    ...
    $id_contractor = mysqli_real_escape_string($db->query, $_POST['id_contractor']);
    ...
    $manager_job = explode(',', mysqli_real_escape_string($db->query, $_POST['manager_job']));
    ...
    if (!mysqli_num_rows($exist_user) > 0) {
        ...
        if (!mysqli_num_rows($db->select('tb_user', 'email = "' . $email . '"', 'id_user', 'ASC')) > 0) {
            ...
            $result = $db->insert('tb_manager_contractor', 'id_manager_contractor = "' . $unique_id . '", name = "' . $name_manager . '", email = "' . $email . '", no_telp = "' . $no_telp . '", id_estate_cordinator = "' . $id_cordinator . '", id_contractor = "' . $id_contractor . '"');
            ...
            if ($result_user) {
                // insert akses
                $data_auth = $db->select('tb_authorization', 'status = "MANAGER KONTRAKTOR"', 'id_auth', 'ASC');
                ...
                if (mysqli_num_rows($data_auth) > 0) {
                    ...
                    if ($update) {
                        ...
                        foreach ($manager_job as $value) {
                            ...
                            $result_job = $db->insert('tb_manager_contractor_job', 'id_manager_contractor_job = "' . $unique_id_job . '", id_manager_contractor="' . $unique_id . '",id_category="' . $data_return['id_category'] . '"');
                        }
