...
if (!empty($obj['usernameOrIpl']) && !empty($obj['password']) && !empty($obj['token'])) {
	...
	if ($jum > 0) {
		...
		if (mysqli_num_rows($cek_auth) > 0) {
			...
			if ($status_auth == 'WARGA' || $status_auth == 'BETA TEST') {
				...
				// cek token terlebih dahulu ada atau tidak
				$cek = $db->select('tb_user_fcm_token', 'id_user="' . $u['id_user'] . '"', 'id_fcm', 'ASC');
				if (mysqli_num_rows($cek) > 0) {
					$db->update('tb_user_fcm_token', 'token="' . $token . '"', 'id_user="' . $u['id_user'] . '"');
				} else {
					$db->insert('tb_user_fcm_token', 'id_user="' . $u['id_user'] . '", token="' . $token . '"');
				}
