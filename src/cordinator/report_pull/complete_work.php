...
if(!empty($_POST['id_report']) && !empty($_POST['id_estate_cordinator']) && !empty($_POST['finish_time']) && !empty($_POST['duration']) && !empty($_POST['message'])) {
	...
	if(isset($ubah1) && isset($ubah2)) {
		$r = $db->update('tb_process_work_cordinator', 'photo_complete_1="'.$ubah1.'"', 'id_report="'.$id_report.'"');
		$r = $db->update('tb_process_work_cordinator', 'photo_complete_2="'.$ubah2.'"', 'id_report="'.$id_report.'"');
	} else if (isset($ubah1)) {
		$r = $db->update('tb_process_work_cordinator', 'photo_complete_1="'.$ubah1.'"', 'id_report="'.$id_report.'"');
	} else if(isset($ubah2)) {
		$r = $db->update('tb_process_work_cordinator', 'photo_complete_2="'.$ubah2.'"', 'id_report="'.$id_report.'"');
	...
	$body = 'Laporan anda "' . $id_user['description'] .'" telah selesai dikerjakan';
	...
	if($r && $rsreport && $rsprocess && $rswork && $status) {
