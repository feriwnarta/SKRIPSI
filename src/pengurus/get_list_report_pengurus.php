    ...
    if ($status == 'Semua') {
        ...
        while ($result_report = mysqli_fetch_assoc($data_report)) {
            ...
            while ($result_category = mysqli_fetch_assoc($data_category)) {
                ...
                $em = $db->select('tb_estate_manager', 'id_estate_manager NOT IN ("")', 'id_estate_manager', 'ASC');

                if(mysqli_num_rows($em) > 0) {
                    $em = mysqli_fetch_assoc($em);
                    $data_report_return[$index]['pic'][] = $em;
                }
