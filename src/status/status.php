...
if (!empty($obj['id_user'])) {
	...
	if ($jum > 0) {
		...
		while ($value = mysqli_fetch_assoc($cek)) {
			...
			// query check tb setting socmed
			$checkSetting = $db->select('tb_setting_socmed', "id_status = '{$id_status}' AND report_user_id = '{$id_user}' AND (report NOT IN ('1') OR not_interested NOT IN ('1') )", 'create_at', 'ASC');
